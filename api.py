...
def get_scientific_details(recommendation_type, recommendation):
    """Get scientific details for the recommendation"""
    
    if recommendation_type == "crop":
        crop_name = recommendation.lower()
        
        # Return crop-specific scientific details if available
        if crop_name in CROP_RECOMMENDATIONS:
            return CROP_RECOMMENDATIONS[crop_name]
        
        # For crops without specific details, return generic information
        return {
            "description": f"{recommendation} is a widely cultivated crop with specific soil and climate requirements.",
            "optimal_conditions": {
                "N": "Moderate-high", "P": "Moderate", "K": "Moderate", 
                "pH": "5.5-7.0", "temperature": "20-30°C"
            }
        }
    else:  # fertilizer recommendation
        fert_name = recommendation.lower()
        
        # Return fertilizer-specific scientific details
        if "urea" in fert_name:
            return {
                "description": "Urea is a high-nitrogen fertilizer that rapidly converts to ammonium carbonate in the soil.",
                "composition": "46-0-0 (N-P-K)",
                "best_use": "Excellent for leafy vegetables and crops requiring high nitrogen."
            }
        elif "dap" in fert_name:
            return {
                "description": "Diammonium Phosphate (DAP) provides both nitrogen and phosphorus in readily available forms.",
                "composition": "18-46-0 (N-P-K)",
                "best_use": "Excellent for promoting root development and early crop establishment."
            }
        elif "npk" in fert_name:
            return {
                "description": "NPK fertilizers provide balanced nutrition with varying ratios of nitrogen, phosphorus, and potassium.",
                "composition": "Varies (common ratios include 15-15-15, 20-10-10)",
                "best_use": "Good all-purpose fertilizer for a wide range of crops."
            }
        elif "mop" in fert_name or "potash" in fert_name:
            return {
                "description": "Muriate of Potash (MOP) is a common potassium fertilizer that also contains chloride.",
                "composition": "0-0-60 (N-P-K)",
                "best_use": "Excellent for crops requiring high potassium, improves quality and disease resistance."
            }
        else:
            return {
                "description": f"{recommendation} is a specialized fertilizer that addresses specific nutrient needs.",
                "best_use": "Consult local agricultural extension services for application rates and timing."
            }

# API Endpoints
@app.get("/")
async def root():
    return {"message": "Welcome to Deeproot AgriPredict API", "version": "2.0.0"}

@app.get("/info")
async def get_info():
    """Get information about the model and available endpoints"""
    return {
        "model_info": {
            "input_dim": input_dim,
            "crop_classes": crop_classes,
            "fertilizer_classes": fert_classes,
            "feature_names": feature_names,
            "device": str(device)
        },
        "available_crops": CROP_NAMES,
        "available_fertilizers": FERTILIZER_NAMES,
        "endpoints": [
            {"path": "/", "method": "GET", "description": "Root endpoint"},
            {"path": "/info", "method": "GET", "description": "Get information about the model"},
            {"path": "/predict", "method": "POST", "description": "Get predictions from natural language query"},
            {"path": "/crops", "method": "GET", "description": "List available crops"},
            {"path": "/fertilizers", "method": "GET", "description": "List available fertilizers"}
        ]
    }
...
async def get_fertilizers():
    """Get list of supported fertilizers"""
    return {"fertilizers": FERTILIZER_NAMES}
...
async def predict(data: PromptRequest):
    ...
    try:
        ...
        features, auto_task_id, query_analysis = generate_features(
            data.input, 
            explicit_params=data.soil_params
        )
        ...
        _, top3_crop_indices = torch.topk(crop_probs, min(3, crop_classes), dim=1)
        _, top3_fert_indices = torch.topk(fert_probs, min(3, fert_classes), dim=1)
        
        # Map to human-readable names with confidence
        top3_crops = [
            {"name": get_crop_name(idx.item()), "confidence": crop_probs[0, idx].item()}
            for idx in top3_crop_indices[0]
        ]
        
        top3_fertilizers = [
            {"name": get_fertilizer_name(idx.item()), "confidence": fert_probs[0, idx].item()}
            for idx in top3_fert_indices[0]
        ]
        ...
        if task_id_to_use == 0:  # Crop recommendation
            ...
            rec_type = "crop"
            alternatives = top3_crops[1:] if len(top3_crops) > 1 else []
        else:  # Fertilizer recommendation
            recommendation = get_fertilizer_name(fert_pred)
            confidence = fert_conf
            rec_type = "fertilizer"
            alternatives = top3_fertilizers[1:] if len(top3_fertilizers) > 1 else []
        
        # Extract soil parameters for explanation
        soil_params = {}
        if data.soil_params:
            # Use explicitly provided parameters
            if data.soil_params.N is not None:
                soil_params['n'] = data.soil_params.N
            if data.soil_params.P is not None:
                soil_params['p'] = data.soil_params.P
            if data.soil_params.K is not None:
                soil_params['k'] = data.soil_params.K
            if data.soil_params.pH is not None:
                soil_params['ph'] = data.soil_params.pH
            if data.soil_params.temperature is not None:
                soil_params['temperature'] = data.soil_params.temperature
            if data.soil_params.humidity is not None:
                soil_params['humidity'] = data.soil_params.humidity
            if data.soil_params.rainfall is not None:
                soil_params['rainfall'] = data.soil_params.rainfall
        else:
            # Use parameters extracted from query
        ...
        # Get scientific details
        scientific_details = get_scientific_details(rec_type, recommendation)
